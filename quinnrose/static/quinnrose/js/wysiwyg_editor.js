...
(function( $ ){
	...
	var methods = {
		...
		imageWidget: function(){
			...
			handleFileSelect = function(evt) {
				...
				for (var i = 0, f; f = files[i]; i++) {
					...
					if(!f.type.match('image.*') || !f.name.match(/(?:gif|jpg|png|jpeg)$/)){ // Process
																							// only
																							// Images
		...
		getHTMLTable: function(tblRows,tblColumns,attributes){
			// Function to generate html table. Supplied arguments:
			// tablerows-no.of rows, no.of columns, table attributes.
		...
		init : function( options )
		{
			...
			var statusBar = $("<div/>", {	id : "statusbar",
	       	...
	       	for(var item in menuItems){
	       		if(!settings[item] ){ // if the display is not set to true for
		    ...
		    $(document.body).mousedown(function(event) {
		        ...
		        if (!target.parents().andSelf().is('#context-menu')) { // Clicked
																		// outside
		            $('#context-menu').remove();
		        } 
		        if (!target.parents().andSelf().is('#specialchar') && (target.closest('a').html()!='<i class="fa fa-asterisk"></i>')) { // Clicked
																																		// outside
		        ...
		        if (!target.parents().andSelf().is('#paletteCntr') && (target.closest('a').html()!='<i class="fa fa-font"></i>')) { // Clicked
																																	// outside
		...
		createMenuItem: function(itemSettings, options, returnElement){
			// Function to perform multiple actions.supplied arguments:
			// itemsettings-list of buttons and button options, options: options
			// for select input, returnelement: boolean.
		...
		getSource: function(button, params){
			// Function to show the html source code to the editor and toggle
			// the text display.
			var flag = 0;
		...
		countWords: function(node){
			// Function to count the number of words recursively as the text
			// grows in the editor.
			var count = 0;	
		...
		countChars: function(node){
			// Function to count the number of characters recursively as the
			// text grows in the editor.
			var count = 0;
    		var textNodes = node.contents().filter(function() { 
				return (this.nodeType == 3); 
			});
		...
		showMessage: function(target,message){
			// Function to show the error message. Supplied arguments:target-div
			// id, message-message text to be displayed.
